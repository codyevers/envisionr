...
Load some of the datacubes that we'd like to explore. These are large files so are not included in the EnvisionR package itself. Rather, you'll want to create a folder in your working directory called datacubes then download the following files from box and then move them to the datacubes directory. Download the 7 files in this [box folder](https://oregonstate.box.com/s/lfgqvq0hakprc37n2n7h4xa3kazcft90) and place them in the `datacubes` folder. 

**Load required reference data from the EnvisionR data**

Let's load required reference data into memory. This includes the IDU data as well as boundaries, roads, and cities.
...
Read the datacubes into memory using `read_datacube` and assign each to its own object (e.g., dc1, dc2, etc).
...
Run EnvisionR using `run_envisionr` by providing the object names of the datacubes we just read into memory.
...
-   `data/ref_data.rda` Miscellaneous reference data important to the SWCNH project.
